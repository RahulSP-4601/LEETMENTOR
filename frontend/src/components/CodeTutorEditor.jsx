// src/components/CodeEditorTutor.jsx
...
export default function CodeTutorEditor({
  ...
  const extractPythonBody = (raw) => {
    ...
    for (let i = headerIdx + 1; i < lines.length; i++) {
      ...
      bodyLines.push(ln.slice(headerIndent + 4 > 0 ? headerIndent + 4 : 0)) // typical 4-spaces indent
    }
  ...
  const sanitizeJS = (raw) => {
    ...
    // If a function wrapper is present, extract only the block
    if (/function\s*\(|=>\s*\{/.test(s) || /\bfunction\s+[A-Za-z_]\w*\s*\(/.test(s)) {
      s = stripOuterBraces(s)
      // Some models return "const x = function(...) { ... };" → strip braces again
  ...
  // PY: replace 'pass', type on NEXT line, left col; re-indent body to that level
  const typeBody_Python = async (body) => {
    ...
    // type starting on next line col 1
  ...
  // JS: insert right BEFORE the closing "};"
  const typeBody_JS = async (body) => {
    ...
    const pad = " ".repeat(closeIndent + 2) // body indent inside block
    const reindented = dedent(detab(body)).split("\n").map(ln => (ln ? pad + ln : ln)).join("\n") + "\n"
    await typeAtPosition(closeIdx + 1, 1, reindented)
    return true
  }

  // C++: remove placeholder + 'return {};', type where the placeholder was
  ...
  // Java: replace 'return new int[0];' then insert body there
  ...
  // C: replace "*returnSize = 0;" and "return NULL;"
  ...
  const handleGetCode = async () => {
    ...
    try {
      // always start from fresh DB skeleton
      ...
      let helpers = "" // no helpers unless we really want them
      ...
      if (!ok) {
        ...
        await typeAtPosition(model.getLineCount(), model.getLineMaxColumn(model.getLineCount()), "\n" + dedent(detab(body)) + "\n")
      }
